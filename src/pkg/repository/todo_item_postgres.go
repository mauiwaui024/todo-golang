...
func (r *TodoItemPostgres) GetAll(userId, listId int) ([]todo.TodoItem, error) {
	...
	query := fmt.Sprintf("SELECT EVERYTHING FROM %s tl INNER JOIN %s li on li.item_id = tl.id INNER JOIN %s ul ul.list_id = li.list_id WHERE li.list_id =$1 AND ul.user_id = $2", todoItemsTable, listsItemsTable, usersListsTable)
